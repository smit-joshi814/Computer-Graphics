...
public class P16PivotRotatePolygonCanvas extends Canvas {
    ...
    public void rotatePoly(String xc, String yc, int theta, double px, double py ){
        ...
        for(int angle=0; angle<theta; angle++){
            ...
            //First Row 
            trans[0][0]= Math.cos(rad); trans[0][1]= -Math.sin(rad); trans[0][2]=px*(1.0-Math.cos(rad)) + py*Math.sin(rad);
            //Second Row
            trans[1][0]=Math.sin(rad); trans[1][1]=Math.cos(rad); trans[1][2]=py*(1.0-Math.cos(rad)) - px*Math.sin(rad);
            //Third Row
